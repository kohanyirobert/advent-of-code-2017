...
instance Show Instruction where
  ...
  show (BinaryInstruction {operator = o, operandA = a, operandB = b}) =
    ...
    show o ++ " operandA = " ++ show a ++ " operandB = " ++ show b ++ "}"
  ...
  in if null rcv
       then s {status = Waiting} : ss
                ...
                s' =
                  movePointer
                    1
                    s {status = Running, processor = proc', received = tail rcv}
            in s' : ss
