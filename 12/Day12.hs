...
getPrograms string = map toProgram dataRecords
  where
    toDataLine = \line -> filter (\char -> isDigit char || isSpace char) line
    dataLines = map toDataLine $ lines string
    dataRecords = map words dataLines
    toProgram =
      \(programId:connectedIds) ->
        Program (read programId) (map read connectedIds)
...
buildGraph' seenPrograms program@(Program programId connectedIds) allPrograms =
  Node programId connectedNodes
  where
    newSeenPrograms = program : seenPrograms
    ...
    toNode =
      \connectedProgram ->
        buildGraph' newSeenPrograms connectedProgram allPrograms
...
collectIds' node@(Node id nodes) ids = sort uniqueIds
  where
    allIds = id : foldl (\b a -> b ++ (collectIds' a ids)) ids nodes
    uniqueIds =
      foldl
        (\b a ->
           if a `elem` b
