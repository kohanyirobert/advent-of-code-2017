...
tieKnot start skip list
  | wrap =
    let end = (start + normalizedSkip) `mod` size
  ...
  | otherwise =
    let end = normalizedSkip `mod` size
...
sparseHash times sizes list = result
  where
    (State _ _ result) =
      foldl
        (\state _ -> sparseHash' sizes state)
        (State 0 0 list)
        [0 .. times - 1]
...
knotHashToBin string = foldl (\str num -> str ++ printf "%04b" num) "" nums
  where
    nums = map (\c -> fst . head . readHex $ [c] :: Int) string

knotHash :: Encoding -> String -> String
knotHash Binary string = binaryHash
  where
    hexadecimalHash = knotHash Hexadecimal string
...
knotHash Hexadecimal string = hexaDecimalHash
  where
    sizes = getByteSizes string ++ [17, 31, 73, 47, 23]
