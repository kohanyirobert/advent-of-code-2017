...
getChecksumAfter = read . filter isDigit
...
toStep strings =
  ...
  in \machine ->
       let c = cursor machine
           ...
           s' = getState (branch !! 3)
       in machine {tape = t'', cursor = c', state = s'}
...
getSteps = Map.fromList . map toStateAndStep

getBlueprint :: String -> Blueprint
getBlueprint string =
  let parts = getParts . lines $ string
      detailsPart = parts !! 0
  ...
  in Blueprint
     {startState = startState, checksumAfter = checksumAfter, steps = steps}
...
run bp@(Blueprint {startState = s, checksumAfter = c}) =
  let m = Machine {tape = Map.singleton 0 0, cursor = 0, state = s}
