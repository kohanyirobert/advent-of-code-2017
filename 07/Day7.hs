...
data Detail = Detail String Int Int
  deriving (Eq, Show)

data Tower = Tower Detail [String]
  deriving (Eq, Show)

data Node = Node Detail [Node]
  deriving (Eq, Show)
...
getTowers string = map toTower dataRecords
  where toDataLine = \x -> filter (\y -> isAlphaNum y || isSpace y && not (isControl y)) x
        ...
        toTower = \(name : weight : children) -> Tower (Detail name (read weight) 0) children

isRoot :: Tower -> Towers -> Bool
isRoot (Tower (Detail name _ _) _) towers = all (\(Tower _ children) -> not (name `elem` children)) towers
...
findUnbalanced node@(Node _ children)
  ...
  | otherwise = if matches == []
                then Just node
                else head matches
  where matches = filter isJust $ map findUnbalanced children

getBalancedWeight :: Node -> Int
getBalancedWeight (Node (Detail _ _ _) children) = targetWeight + weightDiff
  where weights = map getWeight children
        minWeight = minimum weights
        maxWeight = maximum weights
        ...
        minSize = length minElems
        maxSize = length maxElems
        targetTotal = if minSize < maxSize then minWeight else maxWeight
        (Node (Detail _ targetWeight _) _) = head $ filter (\(Node (Detail _ _ total) _) -> total == targetTotal) children
