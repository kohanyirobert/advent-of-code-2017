...
stringToRule string =
  let (input:output:[]) =
        map words . map (map slashToSpace) . filter (/= "=>") . words $ string
  in (input, output)
...
quadrantInPatternAt pattern size (i, j) =
  let range = [0 .. size - 1]
  in map
       (\p -> map (\q -> (pattern !! (size * j + q)) !! (size * i + p)) range)
       range
...
patternToGrid pattern =
  let patternSize = length pattern
      size =
        if patternSize `mod` 2 == 0
          then 2
          else 3
      gridSize = patternSize `div` size
      range = [0 .. gridSize - 1]
      coordinates = concat . map (\i -> map (\j -> (i, j)) range) $ range
  in foldl
       (\grid coordinate ->
          Map.insert
            coordinate
            (quadrantInPatternAt pattern size coordinate)
            grid)
       Map.empty
       coordinates
...
gridToPattern grid =
  ...
  in foldl
       (\pattern coordinates ->
          pattern ++ [foldl (rowInGridAt grid) "" coordinates])
       []
       coordinateGroups
...
enhancePattern rules pattern
  | size `elem` [2, 3] =
    let (Just pattern') = Map.lookup pattern rules
    in pattern'
  | otherwise =
    gridToPattern . Map.map (enhancePattern rules) . patternToGrid $ pattern
  where
    size = length pattern
...
countCharInPattern char pattern =
  length . filter (== char) . patternToString $ pattern
