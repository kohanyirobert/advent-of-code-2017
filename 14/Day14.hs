...
getGrid string = Map.fromList $ concat indexedGrid
  where
    ...
    indexedGrid =
      map (\(i, row) -> map (\(j, state) -> ((i, j), state)) $ zip range row) $
      zip range grid
...
updateAdjacents region coordinate@(x, y) grid
  ...
  | otherwise =
    foldl
      (\newGrid adjacentCoordinate ->
         updateAdjacents region adjacentCoordinate newGrid)
      nextGrid
      adjacentCoordinates
  where
    newState = Used region
    ...
    usedNeighbors =
      Map.filter isUsed $
      Map.filterWithKey (\k _ -> k `elem` allNeighbors) nextGrid
    unseenNeighbors =
      Map.toList $ Map.filter (\(Used region) -> region == 0) usedNeighbors
    adjacentCoordinates =
      map (\(adjacentCoordinate, _) -> adjacentCoordinate) unseenNeighbors
...
updateRegions' region grid
  ...
  where
    filtered =
      Map.toList $
      Map.filter (\(Used region) -> region == 0) $ Map.filter isUsed grid
    next = head filtered
    (coordinate, _) = next
...
countRegions grid = region
  where
    (Used region) = maximum . Map.elems . Map.filter isUsed $ grid
