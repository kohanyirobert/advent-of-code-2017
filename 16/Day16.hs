...
data DanceMove = Spin Count | Exchange Position Position | Partner Program Program
  deriving Show

charToSpace :: Char -> Char -> Char
charToSpace target char = if char == target then ' ' else char
...
stringToDanceMove (c : string)
  ...
  where a : b : _ = words . map (charToSpace '/') $ string

getDanceMoves :: String -> [DanceMove]
getDanceMoves string = map stringToDanceMove . words . map (charToSpace ',') . filter (not . isControl) $ string
...
programSwap a b programs
  | a == b = programs
  | otherwise = let size = length programs
                    start = max 0 $ min a b
